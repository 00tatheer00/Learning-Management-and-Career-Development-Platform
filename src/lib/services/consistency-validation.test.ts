...
describe("Step 10 — System-Wide Mathematical Consistency Validation", () => {
  it("guarantees mathematical identity: Phase 1 + Phase 2 + Phase 3 + Phase 4 === All for registrations", async () => {
    const [all, p1, p2, p3, p4] = await Promise.all([
      getCentralPhaseMetrics("all"),
      getCentralPhaseMetrics("phase-1"),
      getCentralPhaseMetrics("phase-2"),
      getCentralPhaseMetrics("phase-3"),
      getCentralPhaseMetrics("phase-4"),
    ]);

    expect(p1.totalEnrollments + p2.totalEnrollments + p3.totalEnrollments + p4.totalEnrollments).toBe(all.totalEnrollments);
    expect(p1.approvedEnrollments + p2.approvedEnrollments + p3.approvedEnrollments + p4.approvedEnrollments).toBe(all.approvedEnrollments);
    expect(p1.pendingEnrollments + p2.pendingEnrollments + p3.pendingEnrollments + p4.pendingEnrollments).toBe(all.pendingEnrollments);
    expect(p1.rejectedEnrollments + p2.rejectedEnrollments + p3.rejectedEnrollments + p4.rejectedEnrollments).toBe(all.rejectedEnrollments);
    expect(p1.estimatedRevenue + p2.estimatedRevenue + p3.estimatedRevenue + p4.estimatedRevenue).toBe(all.estimatedRevenue);
  });
  ...
  it("evaluates registration phase deterministically based on date threshold", () => {
    ...
    const phase4Date = new Date("2026-09-20T10:00:00.000Z");
