...
describe("demo enrollment helpers", () => {
  it("detects demo by enrollment id", () => {
    expect(isDemoEnrollment({ id: "enrollment-demo-web-1", email: "tatheerabidi00@gmail.com" })).toBe(true);
    expect(isDemoEnrollment({ id: "enrollment-demo-web-2", email: "other@example.com" })).toBe(
      true
    );
    expect(isDemoEnrollment({ email: "tatheerabidi00@gmail.com", id: "enr-1" })).toBe(false);
  });
