...
export default function ForgotPasswordForm() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setSent(true);
      toast.success("Check Email", "Reset link sent if account exists.");
    } catch {
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-muted mt-2 text-sm">
            Enter your portal email. We will send a reset link to your email inbox.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-background shadow-xl p-6 sm:p-8">
          {sent ? (
            <Alert variant="success" title="Instructions sent">
              If an account exists for <strong>{email}</strong>, check your email inbox (including spam folder)
