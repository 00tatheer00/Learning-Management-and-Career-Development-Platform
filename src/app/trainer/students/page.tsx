...
export default async function TrainerStudentsPage() {
  ...
  return (
    <div>
      <PortalPageHeader
        title="My Students"
        description="All students enrolled in your courses."
      />

      {students.length === 0 ? (
        <EmptyState title="No students yet" description="Students appear here after admin approves their registration." />
      ) : (
              ...
              <div key={student.id} className="rounded-2xl border border-border bg-background p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted">{program?.title ?? "No course"}</p>
                  </div>
                </div>
